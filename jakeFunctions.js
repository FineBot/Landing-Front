...
function secondDownloadMethod(i, j) {
	execSync(`curl -H \"Authorization: token ${process.env.token}\" -o ./src/images/projects/${i}/${j.match(/([^\/]*)$/gim)[0]} --ssl-no-revoke https://raw.githubusercontent.com/RTUITLab/${i}/master/${j}`, (error) => {
	});
}

async function downloadImages(i, j) {
	let res = execSync(`curl -H "Authorization: token ${process.env.token}" --ssl-no-revoke --header 'Accept: application/vnd.github.v3.raw' -o ./src/images/projects/${i}/${j.match(/([^\/]*)$/gim)[0]} --location https://api.github.com/repos/rtuitlab/${i}/contents/${j}?ref=master`);
	try {
		let buff = JSON.parse(res);
		if (buff.download_url) {
			await download(buff.download_url, `./src/images/projects/${i}/${j.match(/([^\/]*)$/gim)[0]}`);
		} else {
